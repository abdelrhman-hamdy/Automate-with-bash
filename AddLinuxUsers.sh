...
#you should pass the name of the csv file as an argument  
...
while IFS=,  read -ra row_data; do               # row records will be saved in an array called row_data
    if [ $count -eq 0  ]                         # discard the first line as it contains column names not actual data
      then
        ((count++));
        continue;
    fi
 ...
 while true; do
 cut -d: -f1 /etc/passwd | grep $username        # check if the username already exists on the system

 if [  $? -eq 0 ] ; then
   username=iti${row_data[0]}${RANDOM}           # creates new username by generating different random number 
 else
